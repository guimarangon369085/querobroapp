...
async function callBridge(handlerInput, intentName = '') {
  if (!APP_BRIDGE_URL) throw new Error('APP_BRIDGE_URL ausente.');
  if (!APP_BRIDGE_TOKEN) throw new Error('APP_BRIDGE_TOKEN ausente.');
  if (!APP_BRIDGE_HMAC_SECRET) throw new Error('APP_BRIDGE_HMAC_SECRET ausente.');
...
const IntentRequestHandler = {
  ...
  async handle(handlerInput) {
    ...
    const builder = handlerInput.responseBuilder.speak(bridge?.speechText || 'Comando processado.');
    if (!bridge?.shouldEndSession) {
