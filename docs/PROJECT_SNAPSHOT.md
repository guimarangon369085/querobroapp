...
- 2026-04-27: `AMIGAS DA BROA` deixou de ser auto-inativada quando o estoque direto zera. O sincronizador de disponibilidade parou de forçar `active=false` para esses produtos, o catálogo runtime do `/pedido` e do quick create interno passou a manter a amiga visível quando o saldo efetivo está `<= 0`, e a UI agora renderiza esse estado com imagem em tons de cinza, bloqueio de adição e aviso visual `TEMPORARIAMENTE SEM ESTOQUE` no card e no drawer. O backend também deixou de depender do flag `active` para esse bloqueio: `priceOrderItems()` passou a validar saldo real do estoque direto das `AMIGAS` e rejeitar novas inclusões sem saldo, respeitando a reserva do próprio pedido quando a edição é sobre uma ordem já existente. No mesmo dia, a API publicada ganhou auto-reparo dos vínculos legados quebrados entre produto e item de estoque das `Amigas`: quando o produto estiver sem `inventoryItemId`/`inventoryQtyPerSaleUnit`, o catálogo passa a religar o item oficial por nome exato e reaproveitar a gramatura do pack, fazendo o saldo zerado voltar a refletir imediatamente no `/pedido`. Deploys publicados/validados: API `cf99057d-162b-4441-86ac-af745170ab53` + hotfix API `6e2024a3-c52b-4c46-ada2-efac2691e765` e web `65c7e088-11e1-496d-872f-35d033356aa6`, com `pnpm validate:public-deploy` verde e regressão `tests/companion-stock-availability.test.mjs` cobrindo `sem estoque -> bloqueia nova compra -> reposição libera novamente`.

- 2026-04-27: o `/pedido` publicado passou a repassar automaticamente ao cliente a taxa de cartão da integração SumUp no total exibido e no checkout hospedado, sem alterar o valor líquido operacional do pedido dentro do app. O backend agora mantém `order.total` e `payment.amount` internos como base líquida, mas calcula o bruto do cartão com gross-up da taxa online vigente da SumUp (`5,99%`) para o preview público, total ao escolher `CARTÃO`, tela `/pedidofinalizado` e valor aberto na própria página da SumUp. No mesmo dia, o resumo visual do pedido e a tela final passaram a decompor a cobrança com a linha explicita `TAXA DO CARTÃO` entre `FRETE` e `TOTAL`, mantendo o total bruto e o checkout em sincronia. Validação pública concluída: preview publicado `PIX = R$ 57,00` vs `CARTÃO = R$ 60,64`, checagem real em `/pedido` com `Tradicional` `R$ 40,00 -> R$ 42,55` ao alternar para cartão e leitura do aside em produção exibindo `FRETE R$ 0,00`, `TAXA DO CARTÃO R$ 2,55`, `TOTAL R$ 42,55`. Deploys publicados/validados: API `aff8d265-8c32-4493-9552-a55726de8a49`, web `f5d06d60-58b8-4322-98f1-c171477e3033` e refresh visual do web `499eb099-cf59-4dd1-8ad3-359a87e45678`.
...
- Data: 2026-04-27
- Ciclo executado: endurecimento do motor de extrato em `apps/api/src/modules/dashboard/bank-statements.service.ts`, novos testes de contabilidade em `tests/bank-statement-accounting.test.mjs`, `pnpm --filter @querobroapp/api typecheck`, `pnpm --filter @querobroapp/web typecheck`, `node --test tests/bank-statement-import.test.mjs tests/bank-statement-review.test.mjs tests/bank-statement-accounting.test.mjs`, `pnpm --filter @querobroapp/api build`, publicacao no Railway `querobroAPI`, reprocessamento do ultimo extrato publicado e `pnpm validate:public-deploy`.
- Resultado: o dashboard financeiro passou a separar tesouraria, tributos, embalagem, gateway e insumos em plano de contas explicito, e o importador do extrato ganhou uma segunda trilha de conciliacao para `PIX` ja pago usando `valor + paidAt/dueDate/scheduledAt`, mesmo quando `order.customerName` vier vazio ou o nome do pagador divergir do snapshot do pedido. No dado real publicado, o extrato `NU_7707338220_19ABR2026_25ABR2026.csv` saiu de `11` conciliacoes e `22` entradas sem match para `29` conciliacoes e `4` entradas sem match. O web publico permaneceu integro (`home=200`, `pedido=200`, `/pedidos` protegido, `apiHealth=ok`). Deploy API deste lote: `b0041a5a-22e7-4aa5-b137-edbe7bad1f92`.

- Data: 2026-04-27
- Ciclo executado: segunda rodada de correcoes do dashboard em `apps/api/src/modules/dashboard/dashboard.service.ts`, `apps/api/src/modules/dashboard/bank-statements.service.ts`, `apps/web/src/features/dashboard/dashboard-screen.tsx` e `scripts/validate-public-deploy.mjs`; `pnpm --filter @querobroapp/api typecheck`, `pnpm --filter @querobroapp/web typecheck`, `node --test tests/bank-statement-import.test.mjs tests/bank-statement-review.test.mjs tests/bank-statement-accounting.test.mjs`, `pnpm --filter @querobroapp/api build`, `pnpm --filter @querobroapp/web build`, publicacoes no Railway `querobroAPI` e `querobroAPP`, consulta autenticada do resumo publicado e `pnpm validate:public-deploy`.
- Resultado: a leitura do dashboard deixou de misturar dinheiro reconhecido com pagamento historico excedente e deixou explicita a cobertura parcial do extrato. `paidRevenueInRange` agora respeita o teto do `total` de cada pedido, evitando inflacao do recebido por pagamentos duplicados/antigos, e os indicadores comparaveis ao extrato passam a usar apenas a janela realmente coberta quando o periodo selecionado e maior que o extrato importado. No publicado, `Ultimos 30 dias` passou de `Recebido R$ 12.423,60` para `R$ 12.280,72`, enquanto o bloco bancario passou a denunciar cobertura parcial de `19/04/2026` a `25/04/2026` (`7` dias de `31`). Deploys deste lote: API `2c1c40c6-aecf-4dde-984f-a9e039cba1b1` e web `1412d46b-8153-47ca-8dc5-71bfedd92050`.
