...
function normalizeOrderStatus(status?: string | null) {
  ...
  if (normalized === 'CONFIRMADO' || normalized === 'EM_PREPARACAO' || normalized === 'NO_FORNO') return 'ABERTO';
...
function orderRowToneClass(status?: string | null) {
  ...
  return (
    ORDER_ROW_TONE_CLASS_BY_STATUS[normalized] ||
    ORDER_ROW_TONE_CLASS_BY_STATUS.ABERTO
  );
}
...
function orderStatusChipClass(status?: string | null) {
  const normalized = normalizeOrderStatus(status);
  return (
    ORDER_STATUS_CHIP_CLASS_BY_STATUS[normalized] ||
    ORDER_STATUS_CHIP_CLASS_BY_STATUS.ABERTO
  );
}
...
function stripTrailingPostalCode(value: string) {
  return value.replace(/\b\d{5}-?\d{3}\b/g, '').replace(/\s*,\s*$/, '').trim();
}
...
function formatCustomerAddress(order: ConfirmationOrder) {
  ...
  const parsedFromRaw = parseShortAddress(order.customerAddress || order.customerAddressLine1 || '');
  ...
  const complement =
    addressLine2 && addressLine2.toLowerCase() === deliveryNotes.toLowerCase() ? addressLine2 : addressLine2;
...
export default function ConfirmacoesQueueScreen() {
  ...
  const loadQueue = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/internal/orders/daily-digest/preview?date=${encodeURIComponent(dateKey)}`, {
        method: 'GET',
        cache: 'no-store'
      });
      const payload = (await response.json().catch(() => null)) as ConfirmationQueueResponse | { message?: string } | null;
      if (!response.ok) {
        throw new Error((payload as { message?: string } | null)?.message || 'Falha ao carregar a fila de confirmações.');
      }
      setData(payload as ConfirmationQueueResponse);
    } catch (fetchError) {
      setError(fetchError instanceof Error ? fetchError.message : 'Falha ao carregar a fila de confirmações.');
    } finally {
      setLoading(false);
    }
  }, [dateKey]);
  ...
  const updateOrderStatus = useCallback(async (orderId: number, status: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error((payload as { message?: string } | null)?.message || 'Falha ao atualizar o status.');
      }
      const nextStatus = normalizeOrderStatus((payload as { status?: string } | null)?.status || status);
      setData((current) => {
      ...
      setStatusActionError(updateError instanceof Error ? updateError.message : 'Falha ao atualizar o status.');
    } finally {
      setStatusUpdateOrderId(null);
    }
  }, [paymentUpdateOrderId, statusUpdateOrderId]);

  const markOrderPaid = useCallback(async (orderId: number, nextPaid: boolean) => {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as
        ...
        | null;
      if (!response.ok) {
        throw new Error((payload as { message?: string } | null)?.message || 'Falha ao atualizar o pagamento.');
      }
      const nextPaymentStatus = normalizePaymentStatus((payload as { paymentStatus?: string | null } | null)?.paymentStatus);
      setData((current) => {
        if (!current) return current;
        return {
          ...current,
      ...
      setStatusActionError(updateError instanceof Error ? updateError.message : 'Falha ao atualizar o pagamento.');
    } finally {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
      <section className="rounded-[28px] border border-[color:var(--line-soft)] bg-[color:var(--bg-card)] px-5 py-5 shadow-[var(--shadow-soft)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <h1 className="text-3xl font-semibold text-[color:var(--ink-strong)]">{formatDateLabel(dateKey)}</h1>
            <p className="text-sm text-[color:var(--ink-muted)]">
              {loading ? 'Carregando fila...' : `${data?.orderCount || 0} pedido(s) na fila de confirmação`}
            </p>
          </div>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl border border-[color:var(--line-soft)] bg-white px-4 py-3">
            <p className="text-[11px] uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">Pedidos</p>
            <p className="mt-2 text-2xl font-semibold text-[color:var(--ink-strong)]">{data?.orderCount || 0}</p>
          </div>
          <div className="rounded-2xl border border-[color:var(--line-soft)] bg-white px-4 py-3">
            <p className="text-[11px] uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">Entrega / retirada</p>
            <p className="mt-2 text-2xl font-semibold text-[color:var(--ink-strong)]">
              {totals.deliveries} / {totals.pickups}
            </p>
          </div>
          <div className="rounded-2xl border border-[color:var(--line-soft)] bg-white px-4 py-3">
            <p className="text-[11px] uppercase tracking-[0.24em] text-[color:var(--ink-muted)]">Total do dia</p>
            <p className="mt-2 text-2xl font-semibold text-[color:var(--ink-strong)]">{moneyFormatter.format(totals.gross)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[28px] border border-[color:var(--line-soft)] bg-[color:var(--bg-card)] shadow-[var(--shadow-soft)]">
        {statusActionError ? (
          <div className="px-5 pt-5 text-sm text-[color:var(--tone-danger-ink)]">{statusActionError}</div>
        ) : null}
        {error ? (
          <div className="px-5 py-5 text-sm text-[color:var(--tone-danger-ink)]">{error}</div>
        ) : loading ? (
          <div className="px-5 py-5 text-sm text-[color:var(--ink-muted)]">Carregando pedidos para confirmação...</div>
                ...
                {data.orders.map((order) => (
                  <tr key={order.id} className="align-top">
                    {(() => {
                      ...
                      return (
                        <>
                    <td className={`border-y px-4 py-4 text-sm text-[color:var(--ink-strong)] first:border-l first:rounded-l-[18px] ${orderRowToneClass(order.status)}`}>
                      <div className="font-semibold">{formatTime(order.scheduledAt)}</div>
                      <div className="text-[color:var(--ink-muted)]">{formatMode(order.fulfillmentMode)}</div>
                    </td>
                    <td className={`border-y px-4 py-4 text-sm text-[color:var(--ink-strong)] ${orderRowToneClass(order.status)}`}>
                      <div className="font-semibold">{order.customerName}</div>
                      <div className="text-[color:var(--ink-muted)]">{order.customerPhone || 'Sem telefone'}</div>
                    </td>
                    <td className={`max-w-[20rem] border-y px-4 py-4 text-sm text-[color:var(--ink-strong)] ${orderRowToneClass(order.status)}`}>
                      <div className="text-balance text-[color:var(--ink-muted)]">{formatCustomerAddress(order)}</div>
                    </td>
                    <td className={`border-y px-4 py-4 text-sm text-[color:var(--ink-strong)] ${orderRowToneClass(order.status)}`}>
                      ...
                      <div className="mt-1 text-[color:var(--ink-strong)]">{moneyFormatter.format(Number(order.total || 0))}</div>
                    </td>
                    <td className={`border-y px-4 py-4 text-sm ${orderRowToneClass(order.status)}`}>
                      <div className={`relative inline-flex min-w-[8.75rem] items-center rounded-full border shadow-sm transition ${orderStatusChipClass(order.status)} ${statusUpdateOrderId === order.id ? 'opacity-75' : ''}`}>
                        <select
                    ...
                    <td className={`border-y border-r px-4 py-4 text-sm last:rounded-r-[18px] ${orderRowToneClass(order.status)}`}>
                      <div className="flex items-center gap-2">
                        {order.whatsappUrl ? (
                          <>
                            <a
                        ...
                        <button
                          type="button"
                          onClick={() => void markOrderPaid(order.id, !isPaid)}
                          disabled={rowBusy}
                          aria-label={
                            isPaid
                          ...
                          title={isPaid ? `Desmarcar ${order.customerName} como pago` : `Marcar ${order.customerName} como pago`}
          ...
          <div className="px-5 py-5 text-sm text-[color:var(--ink-muted)]">Nenhum pedido programado para esta data.</div>
        )}
      </section>
    </main>
  );
