...
export async function POST(request: Request) {
  ...
  try {
    const response = await fetch(
      `${resolveServerBridgeApiBaseUrl(request, process.env.ORDER_FORM_API_URL)}/orders/intake/google-form/preview`,
      {
        method: 'POST',
        headers,
    ...
    return buildErrorResponse(502, {
      message:
        error instanceof Error
          ? `Falha ao conectar o preview do Google Forms com a API: ${error.message}`
          : 'Falha ao conectar o preview do Google Forms com a API.'
    });
  }
}
