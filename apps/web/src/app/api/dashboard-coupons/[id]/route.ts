...
async function proxyRequest(request: Request, params: { id: string }, method: 'PUT' | 'DELETE') {
  ...
  if (method === 'PUT') {
    try {
      ...
      return buildErrorResponse(400, { message: 'Payload invalido para cupom.' });
    }
  }

  try {
    const response = await fetch(
      `${resolveServerBridgeApiBaseUrl(request, process.env.ORDER_FORM_API_URL)}/dashboard/coupons/${encodeURIComponent(
        params.id
      )}`,
      {
        method,
    ...
    return buildErrorResponse(502, {
      message:
        error instanceof Error
          ? `Falha ao conectar o cupom com a API: ${error.message}`
          : 'Falha ao conectar o cupom com a API.'
    });
  }
}
